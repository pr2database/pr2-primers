...
```{r}
  ...
  primers <- import(here("data/primers.rds"))
  primer_sets <- import(here("data/primer_sets.rds"))  
...
### Supplementary Table - Primer sets stats - Modify for multi column before saving
```{r, eval=FALSE}



latex_table(table = filter(table_primer_sets_ampli, !str_detect(parameter, "ampli_size")),
            caption = "Overall characteristics of primer sets listed in the pr2-primers database.",
            label = "tabsup:primer_sets_ampli",
            file = full_file_name("table_sup_primer_sets_ampli.tex"),
            align = c("l","l","c", "c"),
            scalebox = 1,
            digits= 1
)
...
table <- primers %>% 
  select(primer_id, name, sequence, direction, start_yeast, specificity, reference)
 
add.to.row <- list()
add.to.row$pos <- list(0, 0, 0, 0, 0)
add.to.row$command <- c( 
  # Header for first page
  "id & Name & Sequence & Direction & Start & Specificity & Reference \\\\\n",
  "\\endfirsthead \n \\hline \n",
...
latex_table(table = table,
            caption = "List of 18S rRNA primers in the pr2-primers database ordered by start position relative to the sequence of the yeast \\textit{Saccharomyces cerevisiae} (FU970071).  Some specific primers do not match the yeast sequence and are found at the bottom of the table. DOI for reference can be found in the on-line web application.",
...
```{r, eval=FALSE}
table_amplicon_size <- pr2_match_summary_primer_set %>% 
  filter(kingdom == "Eukaryota") %>% 
  select(primer_set_id, ampli_size_mean)

table <- primer_sets %>%
  left_join(table_amplicon_size) %>% 
  select(primer_set_id, primer_set_name, fwd_name, rev_name, gene_region, ampli_size_mean, specificity, reference) %>% 
...
add.to.row$command <- c( 
  # Header for first page
  "id & Name & Primer fwd & Primer rev & Region & Size & Specificity & Reference \\\\\n",
  "\\endfirsthead \n \\hline \n",
...
latex_table(table = table,
            caption = "List of 18S rRNA primer sets used for metabarcoding in the pr2-primers database.  Size corresponds to the average amplicon size (bp) for sequences from the PR2 database. DOI for reference can be found in the on-line web application.",
...
ggsave(plot= fig_1 , filename="figs/fig_pct_sizes_mismatches_all.pdf",
           width = 19 , height = 20, scale=2, units="cm", useDingbats=FALSE)
...
ggsave(plot= fig , filename="figs/fig_examples_V4_V9.pdf",
           width = 20 , height = 25, scale=1.5, units="cm", useDingbats=FALSE)
...
## Fig. S2 and S5 - Supergroup analysis with size information
...
```
## Fig. S2 and S5 - Supergroup analysis without size information
...
fig_3 <- cowplot::plot_grid(fig_supergroup[[str_c("mismatches", specific_one, sep = " ")]], NULL,
                  ncol = 2, nrow = 1, align = "v" , rel_widths= c(13,0.2))

print(fig_3)

height <- row_height*(ceiling(n_primers[[specific_one]]/ncol))

ggsave(plot= fig_3 , filename=str_c("figs/fig_supergroup_mismatches_", specific_one, ".pdf"),
           width = 20 , height = height, scale=1.75, units="cm", useDingbats=FALSE)
...
ggsave(plot= fig , filename=str_c("figs/fig_primer_sets_position.pdf"),
           width = 10 , height = 15, scale=2.2, units="cm", useDingbats=FALSE)
