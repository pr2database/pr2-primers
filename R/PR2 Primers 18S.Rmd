...
```{r, eval=FALSE}
  ...
  for (i in 1:nrow(primer_sets)) {
    ...
    load(file=str_c("../shiny/data/individual primers/pr2_match_", gene_selected ,rda_file_label, ".rda"))
...
# Replace the gene_region and primer_label_short since they can be changed in the database
  primer_sets_labels <- primer_sets %>% 
    ...
    select(primer_set_id, primer_set_label_short, gene_region, specific, specificity) 
  ...
  pr2_match_final <- pr2_match_final %>% 
    ...
    # remove "_" from primer_set_labels
...
saveRDS(pr2_match_final,
        file=str_c("../shiny/pr2_match_", gene_selected ,"_mismatches_", max_mismatch, "_filtered.rds"))
...
primer_summary <- function(pr2_match, taxo_level){
  summary <-  pr2_match  %>% 
    group_by({{taxo_level}}, gene_region, primer_set_label_long, primer_set_id, specific, specificity ) %>% 
    summarize (n_seq = n(),
...
summary_pos <-  pr2_match  %>% 
    ...
    group_by({{taxo_level}}, gene_region, primer_set_label_long, primer_set_id ) %>% 
...
for (specific_one in c("general", "specific")) {
  ...
  g <- ggplot(pr2_match_summary_primer_set_sg_region) + 
    ...
    scale_fill_viridis_d(option = "inferno") +
    ylim(0,100) +
    facet_wrap(~ primer_set_label_long, scales ="fixed", ncol = ncol) + 
    guides(fill = guide_legend(nrow = 1)) +
    theme(legend.position = "top", legend.box = "horizontal")+ 
    theme(legend.position="none")
...
for (specific_one in c("general", "specific")) {
  ...
  g <- ggplot(pr2_match_summary_filtered_one) + 
    ...
    scale_fill_viridis_d(option = "inferno") +
    ylim(0,100) +
    facet_wrap(vars(primer_set_label_long), scales ="fixed", ncol = ncol) + 
    guides(fill = guide_legend(nrow = 1)) +
    theme(legend.position = "top", legend.box = "horizontal")+ 
    theme(legend.position="none")
