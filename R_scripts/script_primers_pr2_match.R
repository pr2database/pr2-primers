...
##First read in the arguments listed at the command line
  option_list = list(
    make_option(c("-m", "--mismatch"), type="integer", action = "store", default=2, 
                help="Number of maximum mimsatches for each primer [default= %default]", metavar="number"),
  	make_option(c("-s", "--set"), type="integer", action = "store", default=36, 
                help="Primer set [default= %default]", metavar="number"),
  	make_option(c("-g", "--gene"), type="character", action = "store", default="18S_rRNA", 
...
i = 1  
 ...
 # rev_pos <- vmatchPattern(rev, seq, max.mismatch=max_mismatch, min.mismatch=0, with.indels=FALSE, fixed=FALSE, algorithm="auto")
  ...
  pr2_match <- pr2_active %>% 
   ...
   # and are likely to be too short - only do for Eukaryota
           keep_sequence = case_when(  str_detect(gene_region, "V9") & 
                                       !is.na(fwd_pos) & 
                                       !str_detect(str_sub(sequence, start=fwd_pos), "GGATC[AT]") &
                                       kingdom == "Eukaryota" ~ FALSE,
                                       TRUE ~ TRUE)) %>% 
    select(-sequence) %>% 
